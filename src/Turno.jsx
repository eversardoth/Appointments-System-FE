...
const Turnos = () => {
    ...
    useEffect(() => {
        ...
        odontologoService.getAll().then(data => {
            setOdontologos(data.map(odontologo => {
                ...
                let matricula = odontologo.matricula
                return { label: id + '- ' + nombre + ' ' + apellido + ' Matricula: ' + matricula, value: id }
            }))
        })
    ...
    function save() {
        turnoService.save(turno)
            .then((data)=>{
                ...
                setOdontologo(null)
                setPaciente(null)
                turnoService.getAll().then(data => setTurnos(data))
    ...
    let handleTurnos = turnos.map(turn => {
        let pacienteTransitorio = pacientes.find(pac => pac.value == turn.paciente.id)
        let pacienteOk
        if (pacienteTransitorio) {
            pacienteOk = pacienteTransitorio.label.split(" ")[1] + " " + pacienteTransitorio.label.split(" ")[2]
        }
        let odontologoTransitorio = odontologos.find(odonto => odonto.value == turn.odontologo.id)
        let odontologoOk
        ...
        return {
            ...
            pacienteName: pacienteOk,
            odontologoName: odontologoOk,
            pacienteId:turn.paciente.id,
            odontologoId: turn.odontologo.id
        }
    })
    ...
    function update() {
        turnoService.update(selectedTurno).then((data)=>{
            ...
            <div style={{ width: '80%', marginTop: '20px', margin: '0 auto' }}>
                <Dialog header="Actualizar turno" visible={visible} style={{ width: '400px' }} footer={footer} modal={true} onHide={() => setVisible(false)}>
                    <form id="turno-form">
                        ...
                        <div className="p-field">
                            <span className="p-float-label">
                                <Dropdown id="dropdown1" value={selectedTurno.odontologoId} onChange={(e) => {
                                    setSelectedTurno(prevState => {
                        ...
                        <div className="p-field">
                            <span className="p-float-label">
                                <Dropdown id="dropdown2" value={selectedTurno.pacienteId} onChange={(e) => {
                                    setSelectedTurno(prevState => {
            ...
            <div style={{ width: '80%', marginTop: '20px', margin: '0 auto' }}>
                ...
                <Panel header="Turnos">
                    <DataTable value={handleTurnos.sort()} paginator={true} rows="10" selectionMode="single" selection={selectedTurno} onSelectionChange={e => {setSelectedTurno(e.value)}}>
                        ...
                        <Column field="pacienteName" header="Paciente"></Column>
                        <Column field="odontologoName" header="Odontologo"></Column>
                    </DataTable>
                </Panel>
            </div>
