      ...
      <div style={{ width: '80%', marginTop: '20px', margin: '0 auto' }}>
        ...
        <Dialog header="Crear paciente" visible={this.state.visible} style={{ width: '400px' }} footer={this.state.footer} modal={true} onHide={() => this.setState({ visible: false })}>
          <form id="paciente-form">
            ...
            <br />
            <span className="p-float-label">
              <Calendar value={this.state.paciente.fechaIngreso} style={{ width: '100%' }} id="fechaIngreso" onChange={(e) => {
                let val = this.formatDateToString(e.target.value);
                this.setState(prevState => {
                  let paciente = Object.assign({}, prevState.paciente);
                  paciente.fechaIngreso = val
  ...
  showEditDialog = () => {
    this.setState({
      ...
      paciente: {
        ...
        fechaIngreso: this.state.selectedPaciente.fechaIngreso,
        domicilio: {
